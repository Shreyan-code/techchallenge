...
import { Firestore, doc, onSnapshot, DocumentData, DocumentSnapshot, getFirestore } from 'firebase/firestore';
...
export const FirebaseProvider: React.FC<FirebaseProviderProps> = ({
  ...
  useEffect(() => {
    if (!auth) {
      setUserState({ user: null, userProfile: null, isUserLoading: false, userError: new Error("Auth service not provided.") });
      return;
    }
    ...
    const authUnsubscribe = onAuthStateChanged(
      auth,
      (firebaseUser) => {
        // If user logs out, clear profile and stop listening
        if (!firebaseUser) {
          profileUnsubscribe?.();
